...
import { InformacionContacto } from '../../interface';

const Contacto = () => {
	const [infoContacto, setInfoContacto] = useState<Array<InformacionContacto>>(
		[]
	);
					...
					<Centrador>
						...
						<div className={styles.sectionText}>
							...
							<div className={styles.btnCentrador}>
								<BtnLink
									enlace={
										'https://www.facebook.com/profile.php?id=100044996093662'
									}
									nameLink={'Facebook'}
								/>
								<button
									className={styleLink.enlaceStyle}
									onClick={() => changeStateModalOne(!stateModalOne)}
								>
									Donaciones
								</button>

								<button onClick={changeView} className={styleLink.enlaceStyle}>
									Capellania
								</button>
								{/* <BtnLink enlace={"https://www.facebook.com/mario.cortezcortes/"} nameLink={'Correo'}/> */}
							</div>
							{/* MODAL */}
							<Modal state={stateModalOne} changeState={changeStateModalOne}>
								{
									<BodyModal>
										<QR src={IMG} alt='QR' />
									</BodyModal>
